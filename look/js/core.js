...
(function(o) {
     ...
     */
    ND.ajax = function(options, scope) {
        var isError = false
        var url = options.url,
            ajaxUrl = url;
            // 默认区域
        ...
        options.params['_csrf'] =  $('meta[name="csrf-token"]').attr("content");
        ...
        if (url.substring(url.length-1) == '/') {
            ajaxUrl = ajaxUrl.substring(0,ajaxUrl.lastIndexOf('/'))
        }
        //ajaxUrl += '.html'
        ...
        var ajaxObj = {
            ...
            data: options.type === 'GET' ? options.params : options.params,
            // 数据类型
